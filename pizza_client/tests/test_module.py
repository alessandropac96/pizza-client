...
import unittest
from pizza_client.client import ConnectionStatus, LLMClient, LLMClientConfigInput, OpenAIMockClient, QueryResponse, TemperatureInput
...
class TestAbstractLLMClient(unittest.TestCase):
    ...
    def test_query_llm(self):
        async def run_test():
            ...
            print(response)
            self.assertEqual(response, QueryResponse(response='This is a mock response, based on the query: Hello, world! and temperature: temperature=0.5'))
        self.loop.run_until_complete(run_test())
